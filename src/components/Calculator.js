...
class Calculator extends Component {
    ...
    handleCalculator = () => {
        ...
        const bmrValue = Math.round(10 * weight + 6.25 * height - 5 * age + sexValue);
        let activityValue;
        if (activity === '1') {
            activityValue = 1.2;
        } else if (activity === '2') {
        ...
        if ((sex === 'male' || sex === 'female') && height > 0 && weight > 0 && age > 0 && activity > 0) {
            this.setState({
                bmi: bmiValue,
                bmr: bmrValue,
    ...
    handleCaloriesCalculator = () => {
        const { calories, goal, weight, proteinsRatio, carbohydratesRatio, fatsRatio } = this.state;
        ...
        let proteinsValue = Math.round(((proteinsRatio / 100) * goalCaloriesValue) / proteinCalorie);
        let carbohydratesValue = Math.round(((carbohydratesRatio / 100) * goalCaloriesValue) / carboCalorie);

        let proteinsIndexValue = Math.round((proteinsValue / weight) * 10) / 10;
        let carbohydratesIndexValue = Math.round((carbohydratesValue / weight) * 10) / 10;
        let fatsIndexValue = Math.round((fatsValue / weight) * 10) / 10;
        let macronutricients = [Number(proteinsRatio), Number(carbohydratesRatio), Number(fatsRatio)];
        ...
        if (sum === 100 && (goal === 'lose' || goal === 'maintain' || goal === 'gain')) {
            this.setState({
        ...
        } else if (sum !== 100 && (goal === 'lose' || goal === 'maintain' || goal === 'gain')) {
            alert('The sum of all macronutricients must be equal 100%.');
        } else if (sum === 100 && (goal !== 'lose' || goal !== 'maintain' || goal !== 'gain')) {
            alert('Chose your goal to calculate.');
        } else {
            alert('The sum of all macronutricients must be equal 100%. Chose your goal to calculate.');
        }
    };
    ...
    render() {
        ...
        return (
            <>
                ...
                <footer>
                    <p>
                        All calculations are based on Mifflin-St Jeor formula. The calcualtions are only an estimation.
                        If you have any health problems (e.g. diabeters, hypertension) or if you are pregnant, you
                        should contact your doctor or dietician.
                    </p>
                </footer>
            </>
        );
